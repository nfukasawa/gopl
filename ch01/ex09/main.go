...
func Fetch(uri string, w io.Writer) (int, error) {
	uri = complementURI(uri)
	resp, err := http.Get(uri)
	if err != nil {
		return 0, &fetchError{cause: err, kind: fetchErrorKindHTTP}
	}
	defer resp.Body.Close()
	_, err = io.Copy(w, resp.Body)
	if err != nil {
		return 0, &fetchError{cause: err, kind: fetchErrorKindCopy}
	}
...
func main() {
	for _, u := range os.Args[1:] {
		status, err := Fetch(u, os.Stdout)
		if err != nil {
			fmt.Fprintf(os.Stderr, "fetch: %v\n", err)
			os.Exit(1)
		}
		fmt.Fprintln(os.Stdout, "status code:", status)
	}
}
